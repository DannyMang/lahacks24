...
function PostDetails({ title, description }) {
  return (
    <>
      <div className="flex justify-between items-center mt-10">
        <CardItem
          as="p"
          translateZ="60"
          className="text-neutral-500 text-lg font-bold max-w-sm dark:text-white"
        >
          {title}
        </CardItem>
      </div>
...
export default function FeedPost({ mainTitle, username, profileImageUrl, posts }) {
  return (
    <CardContainer className="inter-var">
      <CardBody className="bg-gray-50 relative group/card dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] dark:bg-black dark:border-white/[0.2] border-black/[0.1] w-auto sm:w-[40rem] min-h-[900px] rounded-xl p-6 border-2 border-green-300">
        ...
        {posts.map((post, index) => (
          <PostDetails key={index} title={post.title} description={post.description} />
        ))}
      </CardBody>
    </CardContainer>
  );
